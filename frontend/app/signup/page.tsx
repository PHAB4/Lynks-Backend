...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      if (error.message.includes('already registered')) setServerError('An account with this email already exists.')
      else if (error.message.includes('valid email')) setServerError('Invalid email address.')
              ...
              <div className="flex flex-col gap-1 mt-2">
                {passwordRules.map((rule, i) => (
                  <div key={i} className="flex items-center gap-1.5">
                    {rule.test(form.password) ? <Check size={12} className="text-green-500" /> : <X size={12} className="text-red-400" />}
                    <span className={cn('text-[11px]', rule.test(form.password) ? 'text-green-600' : 'text-[#8B898E]')}>{rule.label}</span>
                  </div>
                ))}
              </div>
            )}
