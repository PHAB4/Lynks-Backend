  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-background px-6">
      <div className="w-full max-w-md">
        <div className="flex flex-col items-center gap-3 mb-8"><h1 className="text-3xl font-bold text-primary">LYNKS</h1><p className="text-text-secondary text-sm">Create your account</p></div>
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div><label className="text-xs font-medium text-text-secondary mb-1 block">Full Name</label><input type="text" value={form.name} onChange={(e) => setForm(f => ({ ...f, name: e.target.value }))} className={cn('w-full py-3 px-4 rounded-xl border bg-surface text-sm text-text-primary focus:outline-none focus:border-primary transition-colors', errors.name ? 'border-red-400' : 'border-border')} placeholder="John Doe" />{errors.name && <p className="text-xs text-red-500 mt-1">{errors.name}</p>}</div>
