    ...
    <AppLayout>
      <div className="flex flex-col h-screen bg-background">
        <div className="flex-1 overflow-y-auto px-6 py-12">
          {messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full gap-[57px] max-w-[880px] mx-auto">
              <p className="text-[40px] font-semibold leading-[50px] text-center text-text-primary">
            ...
            <div className="max-w-[880px] mx-auto space-y-4 py-8">
              {messages.map((msg, i) => (
                <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[70%] rounded-2xl px-4 py-3 text-sm ${msg.role === 'user' ? 'bg-primary text-white' : 'bg-surface border border-border text-text-primary'}`}>{msg.content}</div>
                </div>
              ))}
            </div>
          )}
