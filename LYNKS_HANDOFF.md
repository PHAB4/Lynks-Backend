...
## What is Working (All 18 Core Tests Passing)

| Endpoint | Method | Status | Notes |
|----------|--------|--------|-------|
| `/health` | GET | PASS | Server health check |
| `/auth/signup` | POST | PASS | Creates Supabase Auth user + DB user (trigger) |
| `/profile` | GET | PASS | Returns user profile |
| `/profile` | PATCH | PASS | Updates name, age, country, education, interests |
| `/profile/career-path` | PATCH | PASS | Updates career_path field |
| `/roadmap/generate` | POST | PASS | LLM generates roadmap (5-6 steps, 10-20 tasks) |
| `/roadmap` | GET | PASS | Returns active roadmap with steps and tasks |
| `/roadmap/regenerate` | POST | PASS | Deactivates old roadmap, creates new one |
| `/tasks/{task_id}/evidence` | POST | PASS | Uploads file to Supabase Storage, saves record |
| `/portfolio` | GET | PASS | Returns completed tasks with evidence |
| `/opportunities` | GET | PASS | Returns 30+ Caribbean opportunities |
| `/opportunities?category=X` | GET | PASS | Filters by category (competition, job, etc.) |
| `/chat/message` | POST | PASS | Creates conversation, LLM responds, saves messages |
| `/chat/history` | GET | PASS | Retrieves message history for conversation |
| `/chat/history` | DELETE | PASS | Deletes all conversations and messages |
| 404 handling | GET | PASS | Returns proper 404 for nonexistent routes |
| 401 handling | GET | PASS | Returns 401 without valid JWT |
| 400 handling | GET | PASS | Returns 400 for invalid category filter |

---

## What is Also Working (3 New Features)

| Feature | Endpoint | Status | Notes |
|---------|----------|--------|-------|
| Resume Builder | POST /resume/generate | PASS | LLM generates structured resume from profile + portfolio + roadmap |
| Resume Builder | GET /resume | PASS | Returns user latest resume |
| Enhanced Mentor | POST /chat/message | PASS | Now knows user roadmap, progress, portfolio, past conversations |
| Expanded Opportunities | GET /opportunities | PASS | 30+ opportunities across 5 categories and 8+ Caribbean countries |
...
- **roadmaps** — id (uuid), user_id (uuid FK to users), career_path (text), is_active (bool), created_at
- **steps** — id (uuid), roadmap_id (uuid FK to roadmaps), title (text), description (text), order (int), created_at
- **tasks** — id (uuid), step_id (uuid FK to steps), title (text), description (text), status (text), completed_at (timestamptz), order (int), created_at
- **evidence** — id (uuid), task_id (uuid FK to tasks), user_id (uuid FK to users), file_url (text), file_type (text), file_name (text), verification_status (text DEFAULT pending), uploaded_at (timestamptz DEFAULT now()), created_at
- **conversations** — id (uuid), user_id (uuid FK to users), created_at
- **messages** — id (uuid), conversation_id (uuid FK to conversations), role (text), content (text), tool_calls (json), created_at
- **resumes** — id (uuid), user_id (uuid FK to users), content (jsonb), created_at, updated_at (DEFAULT now())

### Storage Buckets
- **evidence** — public, with INSERT/SELECT policies for all users

### Key SQL Fixes Applied (in Supabase SQL Editor)
...
- agents/mentor.py — Main conversational agent (orchestrator + chat). Knows user full context (profile, roadmap, portfolio, past conversations). Can call other agents as tools (generate_roadmap, get_portfolio, find_opportunities, complete_task)
...
### Windows-specific
- curl does not work well on Windows — use PowerShell Invoke-RestMethod or Swagger UI at http://localhost:8000/docs instead
- Unicode rendering — Windows terminal may garble special characters (a, a, o) — display issue, not a code bug
...
### Code-specific
- annotations field — Groq API does not support the annotations field in messages. The mentor agent uses _sanitize_messages() to strip it before sending to the LLM. If you modify the mentor, always pass messages through _sanitize_messages().
...
- model_dump() includes extra fields — OpenAI SDK choice.message.model_dump() includes annotations, function_call, etc. that Groq does not support. Always sanitize before sending to LLM.
...
3. Enhanced Mentor — now knows user roadmap (with steps/tasks/progress), portfolio (completed tasks + evidence), past conversations (last 3 summaries), and can mark tasks complete from chat
