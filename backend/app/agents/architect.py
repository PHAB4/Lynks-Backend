...
(Groq, Highrise, Impala AI, Ollama). No LangChain or CrewAI dependency.
...
from backend.app.models.db_models import Roadmap, Step, Task, User
...
1. The roadmap must have 4-8 steps (milestones), ordered logically from foundational to advanced.
2. Each step must have 2-4 tasks (concrete, actionable items the user can actually do).
3. Tasks must be specific and Caribbean-contextualized where possible.
...
6. order on both steps and tasks is a 1-based integer.
...
async def save_roadmap(
    ...
    roadmap = Roadmap(id=roadmap_id, user_id=user_id, career_path=profile.career_path, is_active=True)
    db.add(roadmap)

    for step_gen in generated.steps:
        step_id = str(uuid.uuid4())
        step = Step(id=step_id, roadmap_id=roadmap_id, title=step_gen.title, description=step_gen.description, order=step_gen.order)
        db.add(step)
        for task_gen in step_gen.tasks:
            task = Task(id=str(uuid.uuid4()), step_id=step_id, title=task_gen.title, description=task_gen.description, order=task_gen.order, status="pending")
            db.add(task)
