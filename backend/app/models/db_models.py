...
class User(Base):
    ...
    interests: Mapped[list[str]] = mapped_column(ARRAY(Text), nullable=False, default=list)
...
class Roadmap(Base):
    ...
    steps: Mapped[list["Step"]] = relationship(back_populates="roadmap", order_by="Step.order")


class Step(Base):
    __tablename__ = "steps"
    id: Mapped[str] = mapped_column(Text, primary_key=True, default=lambda: str(uuid.uuid4()))
    roadmap_id: Mapped[str] = mapped_column(Text, ForeignKey("roadmaps.id"), nullable=False)
    title: Mapped[str] = mapped_column(Text, nullable=False)
    description: Mapped[str] = mapped_column(Text, nullable=False)
    order: Mapped[int] = mapped_column(Integer, nullable=False)
    roadmap: Mapped["Roadmap"] = relationship(back_populates="steps")
    tasks: Mapped[list["Task"]] = relationship(back_populates="step", order_by="Task.order")


class Task(Base):
    __tablename__ = "tasks"
    id: Mapped[str] = mapped_column(Text, primary_key=True, default=lambda: str(uuid.uuid4()))
    step_id: Mapped[str] = mapped_column(Text, ForeignKey("steps.id"), nullable=False)
    title: Mapped[str] = mapped_column(Text, nullable=False)
    description: Mapped[str] = mapped_column(Text, nullable=False)
    order: Mapped[int] = mapped_column(Integer, nullable=False)
