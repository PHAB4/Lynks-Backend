...
- **Base URL (local dev):** `http://localhost:8000` *(update once deployed)*
...
Request body *(any subset of the fields below — proposal, confirm with team)*:
```json
{ "name": "string", "age": 25, "country": "string", "education_level": "string", "employment_status": "string", "interests": ["string"] }
```
**Editable via this endpoint:** `name`, `age`, `country`, `education_level`, `employment_status`, `interests`.
...
## Roadmap
*// TBD — `roadmaps`, `steps`, and `tasks` tables are proposed in `SCHEMA.md`, not yet
confirmed by the team. Resolved: steps and tasks are two different levels, not one table —
a roadmap has several steps, each step has several tasks. See `SCHEMA.md` for the full
reasoning.*
...
*// TBD — `tasks` and `steps` tables are proposed in `SCHEMA.md`; confirm with the team
...
## Resume Building
*// TBD — `content` shape below is a placeholder; decide plain text vs structured
object (sections, bullets) before building `/resume/generate`.*
...
Success response (201):
```json
{ "resume_id": "16fd2706-8baf-433b-82eb-8c7fada847da", "content": "string or structured object — TBD" }
```
...
*// TBD — exact fields depend on how the Adzuna API response gets mapped/normalized.*

### GET /opportunities
Auth required: Yes

Query params (optional): `location`, `pay`, `age`, `experience`
